...
class TestPrimaryAgent:
    ...
    def dummy_rule(self):
        return models.Rule.model_construct(rule_id="R1", description="desc", type="regex", severity="low", domain="other", pattern="foo")
    def test_generate_plan_success(self):
        with patch.object(primary_agent, "_call_llm", return_value='{"goal":"g","steps":["a","b"]}'):
    ...
    def test_execute_task_aborts_when_not_approved(self):
        plan = PlanSummary(action_plan="do", goal="g", domain="other", sub_actions=["s"], original_prompt="p")
